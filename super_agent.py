        ...
        while i < frames:
            ...
            if i > observation:
                ...
                if(self.neighbouring_buffer):
                    ...
                    minibatch_exchange = random.sample(self.neighbouring_buffer,50)
                    #Gets back the Xtrain and ytrain
                    ...
                    #This started working only after trail and error, so can be one of the potential issues
                    #The training accuracy remains constant
                    Training_X_exchange= np.array(Xtrain_exchange).reshape(50,5)
                    Training_Labels_exchange = np.array(ytrain_exchange).reshape(50,3)
                    time.sleep(0.01)
                    model1.fit(Training_X_exchange, Training_Labels_exchange, batch_size = 50 , epochs = 1 , verbose = 1 )
                    self.neighbouring_buffer=[]
